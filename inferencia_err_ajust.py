...
# Importamos tus módulos de la versión súper ligera
...
def main():
    ...
    print(f"Iniciando Inferencia ELWRYM en: {device}")

    # =========================================================================
    # ⚙️ CONFIGURACIÓN: Cambia las rutas según lo que quieras evaluar
    # =========================================================================
    
    # 1. Ruta del modelo .pth que quieres probar (del 1 al 20)
    RUTA_MODELO_PTH = r"C:\CODE2026\CASSIproy2026\CASSIproy2026\ELWRYMnet\ELWRYM_1a20\pesos_elwrym_1_bloques.pth"
    
    # 2. Ruta exacta de la carpeta de la imagen de prueba
    ...
    # 🧠 DETECCIÓN INTELIGENTE DEL NÚMERO DE BLOQUES
    nombre_archivo_pth = os.path.basename(RUTA_MODELO_PTH)
    match = re.search(r'_(\d+)_bloques', nombre_archivo_pth)
    
    if match:
        num_bloques = int(match.group(1))
        print(f"[*] Detectado modelo de {num_bloques} bloques DFHM desde el nombre del archivo.")
    else:
        # Si por alguna razón el nombre no coincide, por defecto usamos 9
        print("[!] No se pudo detectar el número de bloques en el nombre. Usando 9 por defecto.")
        num_bloques = 9
    ...
    # Inicializar la Red Extra-Ligera con la cantidad exacta de bloques extraída
    modelo = ELWRYMNet(in_channels=1, out_channels=31, num_features=64, num_blocks=num_bloques, S=2).to(device)
    ...
    fig.canvas.manager.set_window_title(f'Inferencia ELWRYM ({num_bloques} Bloques)')
    plt.subplots_adjust(bottom=0.25, top=0.85)

    # Título principal con métricas globales
    fig.suptitle(f"Imagen: {nombre_imagen_objetivo} | ELWRYM {num_bloques} Bloques | PSNR: {psnr_val:.2f} dB | SSIM: {ssim_val:.4f} | SAM: {sam_val:.2f}°", 
    ...
    titulo_pred = axes[2].set_title(f"Predicción ELWRYM ({longitudes_onda[banda_inicial]}nm)")
    img_pred_plot = axes[2].imshow(cubo_pred[banda_inicial], cmap='gray', vmin=0, vmax=1)
    axes[2].axis('off')

    # Detección del máximo error dinámico (el ajuste que hicimos antes)
    error_inicial = np.abs(cubo_gt[banda_inicial] - cubo_pred[banda_inicial])
    max_err_inicial = error_inicial.max() 
    ...
    def update(val):
        ...
        # Ajuste dinámico del error al deslizar
        ...
        titulo_pred.set_text(f"Predicción ELWRYM ({wl}nm)")
